...
public class ShortenUrlStoreTests : IAsyncLifetime
{
    private ShortenPostgresDbContext _dbContext = null!;
    private ShortenUrlPostgresStore _store = null!;
    ...
    public Task InitializeAsync()
    {
        ...
        _store = new ShortenUrlPostgresStore(_dbContext);
    ...
    public async Task CreateAsync_ShouldAddShortenUrlToDatabase()
    {
        ...
        // Assert
        var result = await _dbContext.ShortenUrls.FirstOrDefaultAsync(x => x.Code == "abc123", cancellationToken);
    ...
    [Fact]
    public async Task UpdateAsync_ShouldModifyShortenUrl()
    {
        // Arrange
        var shortenUrl = ShortenUrl.Create(
            "https://www.example.com/test",
            "https://short.url/upd123",
            "upd123");
        await _store.CreateAsync(shortenUrl, CancellationToken.None);
        await _dbContext.SaveChangesAsync();
        ...
        shortenUrl.IncrementAttemptCount();
        await _store.UpdateAsync(shortenUrl, CancellationToken.None);
        await _dbContext.SaveChangesAsync();

        // Assert
        var result = await _store.GetByCodeAsync("upd123", CancellationToken.None);
        result.Should().NotBeNull();
